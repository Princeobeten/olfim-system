...
export default function Alert({ 
  ...
  return (
    <div className={`border rounded-md p-4 ${variants[variant]} ${className}`}>
      <div className="flex">
        {showIcon && <div className="flex-shrink-0 mr-3">{icons[variant]}</div>}
        <div className="flex-1">{children}</div>
        {onDismiss && (
          <button
            type="button"
            className="ml-auto -mx-1.5 -my-1.5 rounded-md p-1.5 inline-flex text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
            onClick={onDismiss}
          >
